...
/* enableAutoPipelining:false - כבינו במכוון את מנגנון האצווה האוטומטי
   של הספרייה (שקיבץ כמה פקודות Redis לקריאת HTTP אחת ברקע): הוא זה
   שגרם לקריסה התהליכית שתועדה למעלה כשההגדרות חסרות, וגם - בבדיקה
   מול Redis אמיתי - לשגיאת "Request failed" על פקודת SET תמימה
   לגמרי. קריאות ישירות (בלי pipelining) פשוטות יותר לאבחון ואמינות
   יותר בנפח השימוש הקטן שלנו, שבו אין שום יתרון ביצועים מהאצווה. */
const kv = (REDIS_URL && REDIS_TOKEN) ? new Redis({ url: REDIS_URL, token: REDIS_TOKEN, enableAutoPipelining: false }) : null;
function requireKv(){
